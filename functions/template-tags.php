...
function h1_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'h1-theme' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'h1-theme' ),
...
function h1_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && h1_categorized_blog() ) {
			printf( '<span class="cat-links">' . __( 'Posted in %1$s', 'h1-theme' ) . '</span>', $categories_list );
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'h1-theme' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . __( 'Tagged %1$s', 'h1-theme' ) . '</span>', $tags_list );
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( __( 'Leave a comment', 'h1-theme' ), __( '1 Comment', 'h1-theme' ), __( '% Comments', 'h1-theme' ) );
		echo '</span>';
	}

	edit_post_link( __( 'Edit', 'h1-theme' ), '<span class="edit-link">', '</span>' );
}
endif;

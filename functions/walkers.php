 ...
 *
 * 		'walker' => new H1_Walker_Nav_Menu( 'prefix', 'use_sublink' )
 *
...
class New_H1_Walker_Nav_Menu extends Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		$output .= "\n$indent<ul class=\"{$this->h1_custom_prefix}__submenu\">\n";
	}
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ( false == $this->h1_sublink_naming ) || ( true == $this->h1_sublink_naming && 0 == $item->menu_item_parent ) ) {

			// Item
			$classes[] = "{$this->h1_custom_prefix}__item";

			// Has children
			if ( in_array( 'menu-item-has-children', $item->classes ) ) {
				$classes[] = "{$this->h1_custom_prefix}__item--has-children";
			}

			// Current item
			if ( in_array( 'current-menu-item', $item->classes ) ) {
				$classes[] = "{$this->h1_custom_prefix}__item--current";
			}

			// Current ancestor
			if ( in_array( 'current-menu-ancestor', $item->classes ) ) {
				$classes[] = "{$this->h1_custom_prefix}__item--current-ancestor";
			}

			// Current parent
			if ( in_array( 'current-menu-parent', $item->classes ) ) {
				$classes[] = "{$this->h1_custom_prefix}__item--current-parent";
			}

			// Home link
			if ( in_array( 'menu-item-home', $item->classes ) ) {
				$classes[] = "{$this->h1_custom_prefix}__item--home";
			}
		} else {

			// Item
			$classes[] = "{$this->h1_custom_prefix}__subitem";

			// Has children
			if ( in_array( 'menu-item-has-children', $item->classes ) ) {
				$classes[] = "{$this->h1_custom_prefix}__subitem--has-children";
			}

			// Current item
			if ( in_array( 'current-menu-item', $item->classes ) ) {
				$classes[] = "{$this->h1_custom_prefix}__subitem--current";
			}

			// Current ancestor
			if ( in_array( 'current-menu-ancestor', $item->classes ) ) {
				$classes[] = "{$this->h1_custom_prefix}__subitem--current-ancestor";
			}

			// Current parent
			if ( in_array( 'current-menu-parent', $item->classes ) ) {
				$classes[] = "{$this->h1_custom_prefix}__subitem--current-parent";
			}

			// Home link
			if ( in_array( 'menu-item-home', $item->classes ) ) {
				$classes[] = "{$this->h1_custom_prefix}__subitem--home";
			}
		}
		...
		// Add link class
		if ( ( false == $this->h1_sublink_naming ) || ( true == $this->h1_sublink_naming && 0 == $item->menu_item_parent ) ) {
			$item_output .= '<a class="' . $this->h1_custom_prefix . '__link" '. $attributes .'>';
		} else {
			$item_output .= '<a class="' . $this->h1_custom_prefix . '__sublink" '. $attributes .'>';
		}
 ...
 *
 * Usage: wp_list_categories( 'walker' => new H1_Walker_Category( $prefix ) );
 *
 * @uses Walker_Category
 */
class H1_Walker_Category extends Walker_Category {
